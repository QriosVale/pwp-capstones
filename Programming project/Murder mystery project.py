...
def get_average_sentence_length(text):
    ...
    new_text_2 = new_text.replace("?",".")   # not as pretty as a for loop but it works for the two variable i got to replace.
    lists = []
    lists.append(new_text_2.split(".")) # this makes lists in a list
    
    length_of_all_sentences = 0
    for x in lists :
        length_of_all_sentences += len(x)
    average_sentence_length = length_of_all_sentences/ len(lists) 
    
    return average_sentence_length
    
# print(get_average_sentence_length(lily_trebuchet_intro))    Testing out my function
Myrtle_Beech_ave_sen_len = get_average_sentence_length(myrtle_beech_intro)
Lily_Trebuchet_ave_sen_len = get_average_sentence_length(lily_trebuchet_intro)
Gregg_T_Fishy_ave_sen_len = get_average_sentence_length(gregg_t_fishy_intro)

print(Myrtle_Beech_ave_sen_len,Lily_Trebuchet_ave_sen_len,Gregg_T_Fishy_ave_sen_len)
...
class TextSample:
  ...
  def __repr__(self):
    return ("The author of this text is {} and the average sentence length is {}".format(self.author, self.average_sentence_length))
  ...
  def prepare_text(text):
    ...
    new_text = []
    new_text = new_string.split()
    return new_text

  def build_frequency_table(new_text):
      frequency_table = {}
      for x in new_text:
...
#testing1234 = TextSample(murder_note, "the murderer")


#newclass = TextSample(myrtle_beech_intro, "Myrtle Beech")
#print(newclass)
...
testing123 = prepare_text(murder_note)
print(testing123)
testing456 = prepare_text(lily_trebuchet_intro)
...
testing543 = build_frequency_table(testing123)
print(testing543)
testing999 = build_frequency_table(testing456)
print(testing999)
...
def ngram_creator(text_list):
    ...
    return ngram_list
testing900 = ngram_creator(testing123)
...
Testing45 = frequency_comparison(testing543, testing999)
print(Testing45)             
...
def percent_difference(sample1, sample2):
    percent = abs(sample1-sample2) / ((sample1 + sample2)/2)
    return percent
    
    
somevalue = percent_difference(Myrtle_Beech_ave_sen_len, Lily_Trebuchet_ave_sen_len)
print(somevalue)
#Myrtle_Beech_ave_sen_len
...
MURDERER = TextSample(murder_note, "Murderer")
MB = TextSample(myrtle_beech_intro,"Myrtle Beech")
LT = TextSample(lily_trebuchet_intro, "Lily Trebuchet")
GTF = TextSample(gregg_t_fishy_intro,"Gregg T Fishy")
contestants = [MB, LT, GTF]

def find_text_similarity(Murderer_Sample, textsample):
    
    sentence_length_difference = percent_difference(Murderer_Sample.average_sentence_length, textsample.average_sentence_length)
    sentence_length_similarity = abs(1 - sentence_length_difference)
    
    word_count_similarity = frequency_comparison(Murderer_Sample.word_count_frequency, textsample.word_count_frequency)

    #ngram_similarity = textsample.word_count_frequency(textsample.ngram_frequency) - Murderer_Sample.word_count_frequency(Murderer_Sample.ngram_frequency)
    ngram_similarity = frequency_comparison(build_frequency_table(MURDERER.ngram_frequency), build_frequency_table(textsample.ngram_frequency))
        
    similarities = (sentence_length_similarity + word_count_similarity + ngram_similarity) / 3 
    
    return similarities

    

for suspect in contestants:
  print(find_text_similarity(MURDERER, suspect))
...
for suspect in contestants:
    print(suspect.author + " has a score of " + str(find_text_similarity(MURDERER, suspect)))
...
print("Gregg T Fishy probably did it.")
